...
- **Live:** https://reodite.com (Cloudflare Pages; custom domain `reodite.com`, registered with Cloudflare)
...
- **Deploy:** GitHub Actions workflow re-runs the embedding pipeline on every push (Hugging Face cache speeds it up to ~1–2 min after the first run), then `npm install && npm run build` (with `VITE_BASE_PATH=/`), then publishes `web/dist/` to **Cloudflare Pages** (project `reodite`) via `cloudflare/wrangler-action` (`wrangler pages deploy`). The heavy Python/torch build stays in Actions — Cloudflare just receives the finished static `dist`. Auth is repo secrets `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`. The custom domain `reodite.com` (apex) is attached to the Pages project in the Cloudflare dashboard; `www.reodite.com` 301-redirects to the apex via a zone Redirect Rule.
...
- **Two top-level routes, hand-rolled router** (`web/src/lib/router.ts`): `/home` (the landing page, rendered WITHOUT the sidebar) and `/app` (the sidebar shell that hosts the chat/lookup/prereq/planning/other panels via the persisted `view` flag — `view` no longer has a `'home'` member). The bare base URL `/` (Vite `base`, now `/` for the root-domain Cloudflare deploy) canonicalizes to `/home`. It's a ~40-line History-API store (`navigate()`/`replaceRoute()`/`useRoute()`), not react-router — too little routing to justify the dep. Route names live in the `ROUTES` constant so renaming is one line. **Static hosts have no SPA history-fallback**, so `web/public/_redirects` ships `/* /index.html 200` — Cloudflare Pages' native SPA rewrite that serves the app shell with a 200 for any path that isn't a built asset (real assets still match first), so a hard refresh / direct hit on a client route like `/app` resolves. The router then reads `location.pathname` and renders the right route (asset URLs are base-absolute, so it works from any depth). **Do not re-introduce a `404.html`** (the old GitHub Pages trick of copying `index.html` → `dist/404.html`): on Cloudflare Pages a `404.html` SHADOWS the `_redirects` 200 rewrite and forces a 404 status on every deep link — which is why the `spa-404-fallback` Vite plugin was removed. The store's persist `version` was bumped to 2 with a `migrate` that maps a leftover `view: 'home'` onto `'chat'`.
...
# to Cloudflare Pages (project `reodite` → https://reodite.com).
git push
```
