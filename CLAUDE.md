...
- **Two top-level routes, hand-rolled router** (`web/src/lib/router.ts`): `/home` (the landing page, rendered WITHOUT the sidebar) and `/app` (the sidebar shell that hosts the chat/lookup/prereq/planning/other panels via the persisted `view` flag — `view` no longer has a `'home'` member). The bare base URL `/` (Vite `base`, now `/` for the root-domain Cloudflare deploy) canonicalizes to `/home`. It's a ~40-line History-API store (`navigate()`/`replaceRoute()`/`useRoute()`), not react-router — too little routing to justify the dep. Route names live in the `ROUTES` constant so renaming is one line. **Static hosts have no SPA history-fallback**, so two mechanisms cover a hard refresh / direct hit on a client route like `/app`: (1) `web/public/_redirects` ships `/* /index.html 200`, Cloudflare Pages' native SPA rewrite (static assets still match first); (2) the `spa-404-fallback` plugin in `vite.config.ts` copies the built `index.html` → `dist/404.html` as a belt-and-suspenders fallback that also keeps `vite preview` working. The router then reads `location.pathname` and renders the right route (asset URLs are base-absolute, so both work from any depth). Don't delete `_redirects` or the 404 copy or a direct hit on `/app` breaks. The store's persist `version` was bumped to 2 with a `migrate` that maps a leftover `view: 'home'` onto `'chat'`.
