...
- **Citation surfacing** — `buildSystemPrompt()` in `web/src/lib/prompts.ts` injects `RESPONSE_INSTRUCTIONS` (or `EASTER_RESPONSE_INSTRUCTIONS` for single-easter retrievals) which require `[N]` citations matching the bracketed numbering in `buildContext`; `ChatMessage` parses them, renders inline superscript chips linking to the chunk's UBC URL, and splits the sources panel into "Sources used" vs "Other retrieved context."
...
- **Citation + rules contract in the system prompt** — `web/src/lib/prompts.ts` exports `buildSystemPrompt(chunks, bareSubject)`, which composes `BASE_SYSTEM_PROMPT` with `RESPONSE_INSTRUCTIONS` (default), `EASTER_RESPONSE_INSTRUCTIONS` (single-easter retrievals), or nothing extra (bareSubject). The rules + `[N]` citation contract used to live at the *user message tail* — that worked on Gemma but Qwen 2.5 1.5B treats user-tail content as content and paraphrases the rules back into its reply ("…no source above is relevant…"). System-role placement gets ChatML's instruction treatment and stops the leak. `ChatMessage.tsx` parses the `[N]` markers — don't strip those rules thinking they're filler, the UI's "Sources used" panel goes silent if you do.
